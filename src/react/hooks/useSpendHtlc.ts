...
import { SpendHtlcParams } from "../../types"
import { useAccount } from "./useAccount"
import { useNetwork } from "./useNetwork"
import { getAddressesHash } from "../../utils"

/**
 * Hook for spending (claiming) Mintlayer HTLC transactions
 * @returns A mutation object for spending HTLC transactions that can be used with React Query
 * @throws {MintlayerClientNotFoundError} If the Mintlayer client is not initialized
 */
export function useSpendHtlc() {
  ...
  return useMutation({
    mutationFn: async (params: SpendHtlcParams) => {
      if (!client) throw new MintlayerClientNotFoundError()
      const response = await client.spendHtlc(params)
